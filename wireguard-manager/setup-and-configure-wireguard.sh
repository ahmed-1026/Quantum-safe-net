...
function installConfs() {
    echo "Setting up the WireGuard server with default configuration..."
    echo ""
    ...
    if [[ -z ${SERVER_PUB_IP} ]]; then
        echo "Error: Unable to detect a public IP address. Exiting."
        exit 1
    fi
    ...
    echo "WireGuard Server Port: ${SERVER_PORT}"
    echo "DNS for Clients: ${CLIENT_DNS_1}, ${CLIENT_DNS_2}"
    echo "Allowed IPs: ${ALLOWED_IPS}"
    echo ""

    echo "Configuration completed. Proceeding with the setup..."
}
